...
model = torchvision.models.detection.fasterrcnn_resnet50_fpn(pretrained=False,num_classes=18)
# For training
...
file.close()
json_dir2='like.json'
file=open(json_dir2, 'r', encoding='utf-8')
s2 = json.load(file)
file.close()
dataset = Image.get_MRCNN_json(s)
dataset2 = Image.get_MRCNN_json(s2)

all_patients = len(dataset.image_info)
print(all_patients)
all_patients2 = len(dataset2.image_info)
print(all_patients2)
images=[]
targets = []
ALL_COUNT=0
...
for i in range(all_patients):
    ...
    print("all_patients"+str(i))

for i in range(all_patients2):
    image = dataset2.image_info[i]['image']
    image=Image.to_torch(image)
    image = image.to(device)
    images.append(image)
    ...
    num_masks=17
    for m in range(num_masks):
        mask = (dataset2.image_info[i]["mask"]==((m+1)*15))
        #print(np.array(mask).shape)
        if mask.max() < 0.5:
            continue
        if i % 10 != 9:
...
for epoch in range(n_epoch):
    ...
    for m in range(65):
        ...
        if(m ==64):
            ...
            dr_loss.append(losses.detach().cpu().float())
    model.eval()
    error=0
    s=0
    for m in range(450):
...
_,ax1=plt.subplots()
ax2=ax1.twinx()
ax1.plot(x,dr_loss, color="red",label='train loss')
ax2.plot(x,dr_train, color="blue",label='train Acc')
ax2.plot(x,dr_val, color="green",label='val Acc')
ax1.set_xlabel('epoch')
ax1.set_ylabel('train loss')
ax2.set_ylabel('train_val Acc')
ax1.legend(loc='upper right')
ax2.legend(loc='upper left')
plt.savefig(fname="trainning.png")
        ...
        for pre in predictions:
            ...
            for i in range(18):
                    if(num[i] != -1):
                        ...
                        point_color = (int(labels[i]*205/18), int(labels[i]*250/18) , 255-int(labels[i]*205/18))  # RGB框的颜色，自定
                        thickness = 1
                        lineType = 4
